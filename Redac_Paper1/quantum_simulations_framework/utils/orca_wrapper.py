...
class OrcaRunner:
    ...
    def __init__(self, 
        ...
        if not os.path.exists(self.orca_path):
            logger.warning(f"Orca executable not found at {self.orca_path}. Calculations will fail.")
...
"""
    ...
    def run_calculation(self, input_file: str) -> str:
        ...
        try:
            ...
            if 'cwd' in locals(): os.chdir(cwd)
            raise
        except Exception as e:
            logger.error(f"Error during Orca execution: {e}")
            if 'cwd' in locals(): os.chdir(cwd)
            raise
    ...
    def calculate_reactivity_descriptors(self, 
                                         ...
                                         coords: str,
                                         method: str = "wB97X-D4",
                                         basis: str = "def2-SVP") -> Dict[str, Any]:
        """
        ...
        inp_n = self.generate_input(f"{molecule_name}_N", coords, method, basis, charge=0, multiplicity=1)
        ...
        inp_p1 = self.generate_input(f"{molecule_name}_P1", coords, method, basis, charge=-1, multiplicity=2)
        ...
        inp_m1 = self.generate_input(f"{molecule_name}_M1", coords, method, basis, charge=1, multiplicity=2)
